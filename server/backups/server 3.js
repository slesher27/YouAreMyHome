...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "world_set") {
      ...
      if (i !== 0) {
        console.warn("[WS] world_set rejected (only host p1 may set world). from:", players[i]?.id);
        return;
      }
    ...
    if (msg.type === "world_op") {
      const i = clients.get(ws);
      if (i !== 0) {
        // Only host is allowed to mutate the shared world for now.
        // (Later we can add "requests" from p2 that host validates/applies.)
        console.warn("[WS] world_op rejected (only host p1 may mutate world). from:", players[i]?.id, "op:", msg.op);
        return;
      }
