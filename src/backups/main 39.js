...
if (state.mode === "interior") {
  ...
  if (!isWall && !isDoor) {
    ...
    drawRect(sx + 4, sy + 4, TILE_SIZE - 8, TILE_SIZE - 8, inner);

    // Inside-edge highlight (makes it look like a wall “lip”)
    // Highlight only on the interior-facing edge
    if (y === 0) drawRect(sx + 4, sy + TILE_SIZE - 6, TILE_SIZE - 8, 2, "rgba(255,255,255,0.12)");
    if (y === INTERIOR_H - 1) drawRect(sx + 4, sy + 4, TILE_SIZE - 8, 2, "rgba(255,255,255,0.12)");
    if (x === 0) drawRect(sx + TILE_SIZE - 6, sy + 4, 2, TILE_SIZE - 8, "rgba(255,255,255,0.12)");
    if (x === INTERIOR_W - 1) drawRect(sx + 4, sy + 4, 2, TILE_SIZE - 8, "rgba(255,255,255,0.12)");
  }
  ...
  if (isWindowSpot && isWall) {
    // Glass inset
    drawRect(sx + 12, sy + 12, TILE_SIZE - 24, TILE_SIZE - 24, "#4a79a8");
    drawRect(sx + 13, sy + 13, TILE_SIZE - 26, TILE_SIZE - 26, "rgba(255,255,255,0.20)");

    // Simple cross muntin
    drawRect(sx + TILE_SIZE / 2 - 1, sy + 14, 2, TILE_SIZE - 28, "rgba(255,255,255,0.35)");
    drawRect(sx + 14, sy + TILE_SIZE / 2 - 1, TILE_SIZE - 28, 2, "rgba(255,255,255,0.35)");
  }
