...
if (state.mode === "interior") {
  ...
  if (obj) {
    // ✅ Interior multi-tile proxies (e.g., bed right-half) must block
    if (state.mode === "interior" && obj.id === PASSABLE_PROXY_ID) return true;
    if (state.mode === "interior" && (obj.id === "occupied" || obj.id === "bed")) return false;
...
function placeRuneCluesInChamber(map, chamber, combo, chestX, chestY) {
  ...
  const totalNeeded = clues.reduce((a, c) => a + c.count, 0); // should be 10

  const CRITICAL = new Set(["dungeon_chest", "dungeon_exit", "dungeon_gate"]);

  function getCandidateTiles(allowOverwriteAny = false) {
    const tiles = [];
    for (let y = chamber.y; y < chamber.y + chamber.h; y++) {
      for (let x = chamber.x; x < chamber.x + chamber.w; x++) {
        ...
        // never overwrite critical objects
        if (obj && CRITICAL.has(obj.id)) continue;

        // first pass: prefer empty tiles only
        if (!allowOverwriteAny && obj) continue;

        tiles.push({ x, y });
      }
    }

    // sort near chest so clues cluster naturally
    tiles.sort((a, b) => {
      const da = Math.abs(a.x - chestX) + Math.abs(a.y - chestY);
      const db = Math.abs(b.x - chestX) + Math.abs(b.y - chestY);
      return da - db;
    });
  ...
  for (let y = chamber.y; y < chamber.y + chamber.h; y++) {
    for (let x = chamber.x; x < chamber.x + chamber.w; x++) {
      if (map.objects?.[y]?.[x]?.id === "rune_clue") {
        map.objects[y][x] = null;
      }
    }
  }

  let tiles = getCandidateTiles(false);

  // fallback: allow overwriting non-critical objects if needed
  if (tiles.length < totalNeeded) {
    tiles = getCandidateTiles(true);
  }

  // hard fail protection
  if (tiles.length < totalNeeded) {
    console.warn("[DUNGEON] Not enough tiles for rune clues", {
      totalNeeded,
  ...
  for (const clue of clues) {
    for (let i = 0; i < clue.count; i++) {
      ...
      const spot = tiles[index++];
      if (!spot) return; // safety guard
  ...
  for (let y = chamber.y; y < chamber.y + chamber.h; y++) {
    for (let x = chamber.x; x < chamber.x + chamber.w; x++) {
      if (map.objects?.[y]?.[x]?.id === "rune_clue") placed++;
    }
  }

  if (placed !== totalNeeded) {
    console.warn("[DUNGEON] Rune clue mismatch", { placed, totalNeeded });
  }
}
...
function placePuzzleChestInDungeon(map, chambers) {
  if (!objDef("dungeon_chest")) return;
  if (!Array.isArray(chambers) || chambers.length === 0) return;

  // Prefer a non-spawn chamber if possible
  const pool = chambers.length > 1 ? chambers.slice(1) : chambers.slice(0);

  let safety = 0;
  while (safety++ < 1200) {
    const chamber = pool[randInt(0, pool.length - 1)];
    const spot = randomEmptyFloorInRect(map, chamber, 400);
    if (!spot) continue;
    ...
    const combo = makeUniqueRuneCombo(RUNE_COMBO_LEN);

    map.objects[spot.y][spot.x] = {
  id: "dungeon_chest",
  hp: objDef("dungeon_chest")?.hp ?? 1,
  meta: {
    puzzleChest: true,
    combo
  }
...
// place rune clues in the SAME chamber
placeRuneCluesInChamber(map, chamber, combo, spot.x, spot.y);

return;

  }

  console.warn("[DUNGEON] Failed to place puzzle chest.");
}
...
if (state.music.ambienceAudio) {
 ...
 // --- Fade out current music, switch to a random dungeon track, fade back in ---
  ...
  state.players[0].x = sx; state.players[0].y = sy; state.players[0].fx = sx; state.players[0].fy = sy; state.players[0].path = [];
  state.players[1].x = sx + 1; state.players[1].y = sy; state.players[1].fx = sx + 1; state.players[1].fy = sy; state.players[1].path = [];
...
playRandomMusicForContext(true);
  ...
  const rx = ret?.x ?? 2;
  const ry = ret?.y ?? 2;
  state.players[0].x = rx; state.players[0].y = ry; state.players[0].fx = rx; state.players[0].fy = ry; state.players[0].path = [];
  state.players[1].x = rx + 1; state.players[1].y = ry; state.players[1].fx = rx + 1; state.players[1].fy = ry; state.players[1].path = [];
