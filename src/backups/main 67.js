...
if (msg.type === "welcome") {
  ...
  // If we have a saved world locally and it's valid, seed it with a NUMERIC id
  if (payloadRaw && isValidWorldPayload(payloadRaw)) {
    const seedId = widNum ?? Date.now();
    if (widNum == null) {
      // Re-home the saved payload under a numeric id so server will accept it
      setActiveWorldId(seedId);
      saveWorldPayload(seedId, payloadRaw);
    }

    console.log("[NET] seeding server world from local save:", seedId);
    sendNet({ type: "world_set", worldId: seedId, world: payloadRaw });
    return;
  }

  // If there's a saved payload but it's invalid, regenerate and seed with numeric id
  if (payloadRaw && !isValidWorldPayload(payloadRaw)) {
    const seedId = widNum ?? Date.now();
    console.warn("[NET] local saved world is invalid; regenerating and reseeding:", seedId);
  ...
  // No local saved payload:
  // - If host: generate/seed a world so ops will broadcast
  // - If non-host: request whatever the server has
  if (state.net.isHost) {
...
function sendWorldOp(op) {
  if (!state.net?.enabled) {
    console.warn("[NET] sendWorldOp skipped (net disabled)", op);
    return;
  }
  if (!state.net.ws || state.net.ws.readyState !== 1) {
    console.warn("[NET] sendWorldOp skipped (ws not ready)", op);
    return;
  }
  console.log("[NET] sendWorldOp ->", op);
  sendNet({ type: "world_op", op });
}

function applyWorldOp(op) {
  if (!op || typeof op !== "object") return;

  // For now, overworld ops only.
  if (op.kind === "set_obj") {
    ...
    // Prevent echo-broadcast if overworld rows are proxied
  ...
  if (op.kind === "set_tile") {
    ...
    // Prevent echo-broadcast if tiles ever get proxied later
    state.net.suppressWorldOps = true;
  ...
  for (let y = 0; y < map.objects.length; y++) {
    ...
    const proxyRow = new Proxy(row, {
      set(target, prop, value) {
        ...
        // ---- CHANGE DETECTION (kills infinite spam) ----
        const prev = target[prop];

        // Treat null/undefined as identical for world objects
        const prevNorm = (prev == null) ? null : prev;
        const nextNorm = (value == null) ? null : value;

        // If nothing actually changed, do NOTHING (no write, no broadcast)
        // - handles endless "set null to null" loops
        // - handles repeated same object reference writes
        if (prevNorm === nextNorm) return true;

        // Local write
        target[prop] = nextNorm;

        // Broadcast only if online and not currently applying a remote op
        if (!state.net?.enabled) return true;
        if (state.net.suppressWorldOps) return true;

// ---- DEBUG: trace spam source (prints once per session) ----
if (!globalThis.__WORLD_OP_TRACE_ONCE) {
  globalThis.__WORLD_OP_TRACE_ONCE = true;
  console.log("[TRACE] First overworld object write that would broadcast:", {
    x, y: yy, prev: prevNorm, next: nextNorm,
    net: { enabled: !!state.net?.enabled, isHost: !!state.net?.isHost, suppress: !!state.net?.suppressWorldOps }
  });
  console.trace("[TRACE] Stack for overworld object mutation");
}
