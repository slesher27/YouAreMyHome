...
buildProjects: {},   // key: "x,y" → { stage, progress }
  ...
  // Crafting
  craftingOpen: false,
  selectedForCraft: new Set(), // item ids selected
  hoveredUI: null,
...
function harvestObject(x, y) {
  ...
  const requiredTool = def.harvest.requiresTool || null;
  if (requiredTool && !hasTool(inv, requiredTool)) return;

  const pName = activePlayer().name;

  for (const [itemId, amt] of Object.entries(def.harvest.gives)) {
    addItem(inv, itemId, amt);

    // Log the action (exclude movement, include harvesting/fishing)
    const itemName = itemDef(itemId)?.name ?? itemId;
    if (itemId === "fish") logAction(`${pName} caught a fish!`);
    else logAction(`${pName} harvested ${amt} ${itemName}${amt === 1 ? "" : "s"}!`);
  }

  obj.hp = (obj.hp ?? 1) - 1;
  if (obj.hp <= 0) map.objects[y][x] = null;
...
function chestLoot(inv) {
  ...
  const tools = ["axe", "hammer", "pickaxe", "fishing_pole", "matches", "saw"];
...
function openMenuForTile(mapX, mapY, screenX, screenY) {
  ...
  const p = activePlayer();
  if (!isAdjacentOrSame(p.x, p.y, mapX, mapY)) return;

  const inv = activeInv();
  const options = [];

  // ---- Campfire special menu ----
  if (obj.id === "campfire") {
    options.push({
      label: "Cook",
      action: () => {
        const cookRecipes = recipeList().filter(r => r.station === "campfire");

        if (cookRecipes.length === 0) {
          openMenu({
            screenX, screenY,
            title: "Cook",
            options: [{ label: "No cook recipes found", disabledReason: "Add recipes with station: campfire" }]
          });
          return;
        }
        ...
        openMenu({
          screenX, screenY,
          title: "Cook",
          options: cookRecipes.map(r => {
            const chk = canCraftRecipe(r);
            return {
              label: r.name ?? `Cook ${Object.keys(r.out ?? {})[0] ?? "item"}`,
              disabledReason: chk.ok ? null : chk.reason,
              action: () => { craftRecipe(r.id); closeMenu(); }
            };
          })
        });
      }
  ...
  if (def.harvest) {
    ...
    if (requiredTool && !hasTool(inv, requiredTool)) {
      disabledReason = `Requires ${itemDef(requiredTool)?.name ?? requiredTool}`;
    }
    options.push({
      label: "Harvest resource",
      disabledReason,
      action: () => harvestObject(mapX, mapY)
    });
  }

  if (def.pickup) {
    options.push({ label: "Pick up", action: () => pickupObject(mapX, mapY) });
  }

  if (def.container) {
    options.push({ label: def.id === "stockpile" ? "Open stockpile" : "Open chest", action: () => openContainerObject(mapX, mapY) });
  }

  if (def.type === "house_door") {
    options.push({ label: "Enter", action: () => tryEnterAt(mapX, mapY) });
  }
...
if (obj && explored) {
  ...
  // label coordinates for house + stockpile
...
function drawCraftingPopup(baseY) {
  const boxW = Math.min(460, window.innerWidth - 20);
  const boxH = 140;
  const x = window.innerWidth - boxW - 10;
  const y = baseY + 80;

  drawRect(x, y, boxW, boxH, "rgba(20,20,20,0.95)");
  ctx.strokeStyle = "rgba(255,255,255,0.12)";
  ctx.strokeRect(x, y, boxW, boxH);

  drawText("Crafting (tap recipe)", x + 10, y + 16, 14);
  ...
  for (const r of matches.slice(0, 6)) {
    const chk = canCraftRecipe(r);
    const line = `${r.name}  (${formatIO(r.in)} → ${formatIO(r.out)})`;

    // clickable area stored for hit test
    r._ui = { x: x + 10, y: yy - 10, w: boxW - 20, h: 20, craftable: chk.ok, id: r.id };

    // hover effect
    const hovering = state.hoveredUI?.type === "recipe" && state.hoveredUI?.id === r.id;
    if (hovering) drawRect(x + 8, yy - 12, boxW - 16, 22, "rgba(255,255,255,0.08)");

    drawText(line, x + 10, yy, 12, "left", chk.ok ? "rgba(255,255,255,0.92)" : "rgba(255,255,255,0.35)");
    if (!chk.ok) drawText(`- ${chk.reason}`, x + 10, yy + 14, 11, "left", "rgba(255,255,255,0.35)");
    yy += chk.ok ? 22 : 28;
  }
}
...
function updateMapCursor(px, py) {
  ...
  // 1) Explicit cursor icon mappings (axe/pick/key etc.)
  ...
  // 2) Pickup-able objects (sticks, stones, etc.) → hand cursor
  if (obj) {
    const def = objDef(obj.id);
    if (def?.pickup) {
...
canvas.addEventListener("pointerup", (e) => {
  ...
  // adjacent object => context menu
  const obj = objectAt(x, y);
  if (obj && isAdjacentOrSame(p.x, p.y, x, y)) {
    openMenuForTile(x, y, px, py);
    return;
  }

  // otherwise pathfind and walk
  setPathTo(x, y);
