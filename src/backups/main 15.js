...
function digAtPlayerTile() {
  ...
  // Don't dig if something is already on the tile (including dropped items)
  if (objectAt(p.x, p.y)) {
    logAction(`${p.name} can't dig here (tile occupied).`);
    return;
  }

  // Roll for critter loot
  if (Math.random() < DIG_CRITTER_CHANCE) {
    const critters = getCritterIdsFromObjects();

    if (critters.length > 0) {
      const critterId = critters[randInt(0, critters.length - 1)];
      const critterObjDef = objDef(critterId);
      const critterName = critterObjDef?.name ?? critterId;

      // Try to add as an item first (if you also defined it in items.json)
      const add = addItem(inv, critterId, 1);

      if (add?.ok) {
        logAction(`${p.name} dug up a ${critterName}!`);
        return;
      }

      // Fallback: drop it on the tile as a dropped_item (needs item def to be pick-uppable)
      const dropped = dropItemOnTile(critterId, 1, p.x, p.y);
      if (dropped) {
        logAction(`${p.name} dug up a ${critterName}!`);
      } else {
        logAction(`${p.name} found something, but couldn't take it.`);
      }
      return;
    }
  }
