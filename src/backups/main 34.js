...
const BUILD_STAGES = [
  ...
  },
  {
    id: "foundation",
    icon: "foundation.png",
    needs: { planks: 200 },       // Foundation -> Framing
    next: "framing"
  },
  {
    id: "framing",
    icon: "framing.png",
    needs: { wood: 100, planks: 100 }, // Framing -> House
    next: "house"
  },
  {
    id: "house",
...
const BUILD_STAMINA_COST_PER_SEC = 0.25;
...
const HUNGER_PER_SEC = 0.35; // hunger gained per second
...
console.log("SEEN AFTER LOAD:", state.tutorial.seen);
...
function drawLeftSidebar() {
  ...
  // Draw button helper
  const drawBtn = (b, icon) => {
    drawRect(b.x, b.y, b.w, b.h, "rgba(20,20,20,0.92)");
...
const TUTORIAL_SLIDES = [
  { title: "Welcome 🙂", text: "Welcome! If this is your first time playing You Are My Home, don't worry- this game is very simple! But here's some tips to get you started..." },
  ...
  { title: "Stamina ⚡️", text: "Moving and performing certain other actions costs stamina. You can rest at any time to recover stamina! Just tap or click on your character to access the character menu, and choose rest! On a keyboard, you can use the R key." },

  { title: "Resources 🪵", text: "There are many items and resources with various purposes scattered accross the map. You can use these items for crafting, cooking, and more! You can harvest resources and pick up items by tapping or clicking on them and choosing from the options in the menu!" },

  { title: "Crafting and Cooking 🛠", text: "Crafting can be done from the Workbench. Cooking can be done from a Campfire or a Stove. You'll find crafting and cooking recipes as you explore, but feel free to try out your own creations!" },
  ...
  { title: "Hunger 🍎", text: "Your character will become hungry after a while, so keep an eye on the hunger meter and be on the lookout for delicious snacks!" },
...
function drawTitleMenu() {
  ...
  // Tutorial checkbox (placed BELOW the message so it can't overlap)
  const cbSize = 16;
  const cbY = msgY + 26;
...
if (state.tutorial.seen) {
  state.title._ui.tutorialToggle = {
    x: cx - 110,
    y: cbY,
    w: 220,
    h: cbSize + 6
...
function handleTitleMenuTap(px, py) {
  ...
  if (ui.startBtn && hitRect(px, py, ui.startBtn)) {
    ...
    // FIRST-EVER PLAY: FORCE TUTORIAL
    if (!state.tutorial.seen) {
...
function updateMusic(dt) {
  ...
  // If we're already in the middle of switching, don't spam new switches
  if (state.music.switching) {
    // But remember where we *should* be (in case it changes again mid-fade)
    state.music.pendingCtxKey = ctxKey;
    return;
  }

  // Context changed? Switch ONCE.
  if (ctxKey !== state.music.contextKey) {
    state.music.pendingCtxKey = ctxKey;
    playRandomMusicForContext(true);
  }
}
...
function placehouseAt(x, y) {
  ...
  state.world.objects[y][x] = { id: "house_door", hp: 999, meta: { interiorId: "house_small" } };
...
function contributeMaterial(proj, itemId, qty) {
  ...
  // remove from inv
  if (!removeItem(inv, itemId, give)) return false;

  // add to mats
  proj.mats = proj.mats || {};
  proj.mats[itemId] = (proj.mats[itemId] || 0) + give;

  pushContribLog(proj, p.name, itemId, give);
  playSound("click");
  return true;
}
...
function withdrawMaterial(proj, itemId, qty) {
  ...
  const take = Math.max(1, Math.min(qty, have));
  const inv = activeInv();
  const ok = addItem(inv, itemId, take);
  if (!ok) { playSound("deny"); return false; }

  proj.mats[itemId] = have - take;
  if (proj.mats[itemId] <= 0) delete proj.mats[itemId];

  const p = activePlayer();
  pushContribLog(proj, p.name, itemId, -take);
  playSound("click");
  return true;
}
...
function constructionUILayout() {
  const pad = 12;
  const boxW = Math.min(860, window.innerWidth - 20);
  ...
  const leftW = Math.floor(boxW * 0.46);
  const rightW = boxW - leftW - pad;
...
state._constructionUI.allBtn = allBtn;
  ...
  for (let i = 0; i < invList.length; i++) {
    ...
    const needLeft = remainingStage[stack.id] || 0;

    const row = { x: lx + 8, y: yy, w: leftW - 16, h: rowH };
    ...
    drawText(`${needLeft}`, row.x + row.w - 90, row.y + 20, 11, "right", "rgba(255,255,255,0.55)");
  ...
  // big stage icon (force the BUILD_STAGES art at full size)
const stageDef = buildStageDef(stageId);
if (stageDef?.icon) {
  const ico = { type: "image", src: `src/icons/${stageDef.icon}` };
  drawCenteredEmoji(ico, rx + 56, ry + 56, 96); // bigger, not tile-sized
}

  // overall progress bar
  const pct = buildOverallProgressPct(stageId, proj.mats);
  drawText(`Overall Progress: ${Math.round(pct * 100)}%`, rx + 120, ry + 30, 12, "left", "rgba(255,255,255,0.85)");

  const pbx = rx + 120;
  const pby = ry + 40;
  const pbw = rightW - 140;
  const pbh = 10;

  drawRect(pbx, pby, pbw, pbh, "rgba(0,0,0,0.55)");
  drawRect(pbx, pby, Math.floor(pbw * pct), pbh, "rgba(120,220,120,0.9)");

  // stockpile list (what’s already deposited)
  drawText("Site Stockpile", rx + 10, ry + 110, 13);
  let sy = ry + 128;
...
// remaining needs (NEXT stage only)
const remainingStage2 = buildRemainingStage(stageId, proj.mats);
drawText("Still Needed (next stage)", rx + 10, ry + 270, 13);

const needKeys = Object.keys(remainingStage2).filter(k => remainingStage2[k] > 0);
let ny = ry + 290;

for (const matId of needKeys.slice(0, 6)) {
  const item = itemDef(matId);
  const label = item?.name || matId;
  drawText(`${label}: ${remainingStage2[matId]}`, rx + 10, ny, 12, "left", "rgba(255,255,255,0.80)");
  ny += 18;
}
if (needKeys.length === 0) {
  drawText("Nothing. Ready to build.", rx + 10, ny, 12, "left", "rgba(120,220,120,0.9)");
}

  // contribution log
  drawText("Contribution Log", rx + 10, ry + 350, 13);
const logBox = { x: rx + 10, y: ry + 368, w: rightW - 20, h: rh - 378 };
  ...
  // store clickable stockpile rows
  state._constructionUI.stockRows = stockRows;

  // build button (when ready for CURRENT stage)
...
const buildLabel = target ? `Build ${stageLabel(target)}` : "Build";
  ...
  if (stageNeeds) {
    ...
    const btn = { x: rx + rightW - 160, y: ry + 12, w: 140, h: 26 };
    drawRect(btn.x, btn.y, btn.w, btn.h, ready ? "rgba(120,220,120,0.25)" : "rgba(255,255,255,0.06)");
    drawText(
  buildLabel,
  btn.x + btn.w / 2,
  btn.y + 17,
  ...
  ready ? "rgba(120,220,120,0.95)" : "rgba(255,255,255,0.75)"
    ...
    state._constructionUI.buildBtn = ready ? btn : null;
  } else {
...
if (ui.allBtn && hitRect(px, py, ui.allBtn)) {
  ...
  for (const matId of Object.keys(remainingStage)) {
    ...
    if (need > 0) contributeMaterial(proj, matId, need);
  }
  return true;
}
...
if (o?.icon) {
  ...
  if (isBuildStage) {
    // Draw big, centered over the whole footprint (so it looks "full sized")
    const size = TILE_SIZE * BUILD_FOOTPRINT;
    const cx = sx + (size / 2);
    const cy = sy + (size / 2);
    drawCenteredEmoji(o.icon, cx, cy, size * 0.9);
  } else {
    drawCenteredEmoji(o.icon, sx + TILE_SIZE / 2, sy + TILE_SIZE / 2, 26);
  }
}
}
...
function upgradeBuildSite(x, y) {
  ...
  const stage = BUILD_STAGES.find(s => s.id === stageId);
...
state.ui.musicDragging = false;
  ...
  if (state.placement?.active && state.placement.dragging) {
    ...
    if (ok) {
      ...
      logAction("Can't place a 6x6 build site there.");
    }
...
if (k === "e") {
  ...
  removeItem(inv, st.id, 1);

  playSound("eat");
  logAction(`${p.name} ate ${def.name ?? st.id}.`);
  return;
}
