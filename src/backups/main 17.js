...
function ensureChatInput() {
  ...
  input.style.font = "14px system-ui";
...
function positionChatInput() {
  ...
  const mapWpx = viewW * TILE_SIZE;
  const mapHpx = viewH * TILE_SIZE;

  const mapLeft = 0;
  const mapTop = UI_TOP_H;

  // Under the map, but above the bottom UI bar
  const desiredY = mapTop + mapHpx + 10;
  const maxY = window.innerHeight - UI_BOTTOM_H - 46; // keep clear of bottom UI
  const y = Math.min(desiredY, maxY);

  const wrapW = 460; // roughly input width + breathing room
  const x = mapLeft + (mapWpx / 2) - (wrapW / 2);
...
function drawChatLog() {
  ...
  const boxW = Math.min(520, mapWpx - 20);
  const boxH = 120;

  const x = mapLeft + (mapWpx / 2) - (boxW / 2);
  const desiredY = mapTop + mapHpx + 54; // below input
  const maxY = window.innerHeight - UI_BOTTOM_H - boxH - 10;
  const y = Math.min(desiredY, maxY);

  drawRect(x, y, boxW, boxH, "rgba(0,0,0,0.55)");
  ctx.strokeStyle = "rgba(255,255,255,0.12)";
  ctx.strokeRect(x, y, boxW, boxH);

  const lines = state.chatMessages.slice(-5); // last 5
  let yy = y + 22;

  for (const m of lines) {
    const name = state.players[m.fromIndex]?.name ?? "???";
    drawText(`${name}: ${m.text}`, x + 12, yy, 13, "left", "rgba(255,255,255,0.9)");
    yy += 20;
  }
}
...
function drawSpeechBubbleAt(centerX, topY, text) {
  ...
  // bubble
  drawRect(x, y, boxW, boxH, "rgba(20,20,20,0.92)");
  ...
  for (const l of lines) {
    drawText(l, centerX, yy, 14, "center", "rgba(255,255,255,0.95)");
    yy += lineH;
  }
}
...
loadSound("hammer", "hammer.mp3", 0.6);
...
function digAtPlayerTile() {
  ...
  if (Math.random() < DIG_CRITTER_CHANCE) {
    ...
    const def = itemDef(critterId);

    const res = addItem(inv, critterId, 1);

    if (res?.ok) {
      logAction(`${p.name} dug up a ${def?.name ?? critterId}!`);
    } else {
      logAction(`${p.name} found a ${def?.name ?? critterId}, but inventory is full.`);
    }
