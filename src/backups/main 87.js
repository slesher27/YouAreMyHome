...
function logAction(text) {
  ...
  // init pending queue (texts we've shown locally but are waiting for server echo)
  state.net = state.net || {};
  ...
  // ONLINE: send to server AND show immediately (so the UI never goes dead)
  if (canSend && !state.net.suppressLogs) {
    // mark as pending so we can ignore the echo
    ...
    // send to server for authoritative ordering + sync
    ws.send(JSON.stringify({ type: "input", payload: { type: "log", text: line } }));
    return;
  }

  // OFFLINE (or applying server log): local append
...
function applyNetLogEntry(entry) {
  ...
  // If this is the server echo of something we already showed locally, swallow it.
  const idx = state.net._pendingLogTexts.indexOf(line);
  if (idx !== -1) {
    state.net._pendingLogTexts.splice(idx, 1);

    // still redraw in case the UI missed the local draw earlier
    renderActivityLogDOM?.();
    return;
  }

  // Otherwise it's from the other player (or server) -> show it
  ...
  // ✅ critical: make the Activity Log UI update on this client
  renderActivityLogDOM?.();
}
...
if (msg.type === "log_init") {
  ...
  if (Array.isArray(msg.entries)) {
    // accept either objects {text} or raw strings, and keep newest-first for your UI
    state.actionLog = msg.entries
      .map(e => (typeof e === "object" && e ? e.text : e))
      .filter(Boolean);

    // server stores oldest->newest; UI draws newest at top
    state.actionLog.reverse();
  }
...
if (msg.type === "snapshot") {
  ...
  if (msg.logs?.length) {
    console.log("[NET] snapshot logs recv", msg.logs.length, msg.logs[msg.logs.length - 1]);
    for (const e of msg.logs) applyNetLogEntry(e);
  }

  if (msg.ops?.length) {
    for (const op of msg.ops) applyNetWorldOp(op);
  }
...
function ensureActivityLogDOM() {
  // Accept either id set, but normalize to activityLog/activityLogBody
  let root =
    document.getElementById("activityLog") ||
    document.getElementById("actionLog");

  let body =
    document.getElementById("activityLogBody") ||
    document.getElementById("actionLogBody");

  // If both exist, great.
  if (root && body) return { root, body };

  // If root exists but body doesn't, create body inside it.
  if (root && !body) {
    body = document.createElement("div");
    body.id = "activityLogBody";
    root.appendChild(body);
  }

  // If root doesn't exist, create full UI.
  if (!root) {
    ...
    root.style.pointerEvents = "none"; // don't steal clicks from the game

    const title = document.createElement("div");
    title.textContent = "Activity";
    title.style.padding = "8px 10px";
    title.style.fontWeight = "700";
    title.style.borderBottom = "1px solid rgba(255,255,255,0.12)";

    body = document.createElement("div");
    body.id = "activityLogBody";
    body.style.padding = "8px 10px";
    body.style.overflowY = "auto";
    body.style.maxHeight = "210px";
...
function renderActivityLogDOM() {
  // Canvas draws the Activity Log. Do not create a second DOM log.
  return;
}
