...
function handleLeftSidebarTap(px, py) {
  ...
  // Settings button opens a simple global settings menu (for now)
  if (ui.settingsBtn && hitRect(px, py, ui.settingsBtn)) {
    playSound("click");

    const sx = UI_LEFTBAR_W + 10;
    const sy = ui.settingsBtn.y + ui.settingsBtn.h / 2;
    ...
    openMenu({
      ...
      options: [
        {
          label: `Music: ${state.music.enabled ? "On" : "Off"}`,
          action: () => {
            state.music.enabled = !state.music.enabled;

            // If turning off, stop audio immediately.
            if (!state.music.enabled && state.music.audio) {
              try { state.music.audio.pause(); } catch (_) {}
            }

            // If turning on and already unlocked, start music.
            if (state.music.enabled && state.music.unlocked) {
