...
function renderActivityLogDOM() {
  // Update an existing Activity Log DOM if present.
  // IMPORTANT: Do NOT create a new floating log UI (that caused the “second log” bug).
  const body =
    document.getElementById("activityLogBody") ||
    document.getElementById("actionLogBody");
  ...
  // newest-first, keep it reasonable so it doesn't become a novel
  const lines = (state.actionLog || []).slice(0, 30);
