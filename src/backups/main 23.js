...
function unlockMusicIfNeeded() {
  ...
  // attempt a "silent" play unlock (some browsers accept this)
  ...
  a.play().then(() => {
    ...
    // If it fails, we'll just keep trying on the next interaction.
    a.volume = prevVol;
  });
}
...
function playRandomMusicForContext(force = false, allowRetry = false) {
  ...
  // already correct context and playing
  ...
  const doSwitch = () => {
    ...
    // set source (mp3 only version)
    a.src = `src/music/${nextName}.mp3`;

    a.currentTime = 0;

    a.play().then(() => {
      // fade in to target volume (slider)
  ...
  if (!a.paused && a.currentTime > 0) {
    ...
    // nothing playing: just switch + fade in
    a.volume = 0;
    doSwitch();
  }
}
