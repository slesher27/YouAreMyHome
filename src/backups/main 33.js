...
const BUILD_STAMINA_COST_PER_SEC = 0.15;
...
const BUILD_ICON_SRC = {
  build_site: "src/icons/build_site.png",
  foundation: "src/icons/foundation.png",
  framing: "src/icons/framing.png",
  house: "src/icons/house.png",
};
...
function drawCenteredImage(img, cx, cy, size) {
  if (!img || !img.complete || !img.naturalWidth) return;
  ctx.drawImage(img, Math.floor(cx - size / 2), Math.floor(cy - size / 2), size, size);
}
  ...
  for (const key of Object.keys(state.buildProjects)) {
    ...
    for (const p of builders) {
      ...
      if (tooFar || p.stamina <= 0) {
        stopBuildTimer(key);
        logAction(`Building interrupted.`);
        return;
      }
...
if (obj.id === "build_site" || obj.id === "foundation" || obj.id === "framing") {
  options.push({
    ...
    action: () => upgradeBuildSite(mapX, mapY)
  });
}
...
const visNow = (state.mode !== "overworld")
  ...
  // Build stages use PNG icons (not emoji)
  if (BUILD_ICON_SRC[obj.id]) {
    const img = getImg(BUILD_ICON_SRC[obj.id]);
    drawCenteredImage(img, sx + TILE_SIZE / 2, sy + TILE_SIZE / 2, TILE_SIZE);
  } else {
