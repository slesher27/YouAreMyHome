...
function drawBuiltInteriorWallsPass(pass) {
  ...
  // Match your existing interior palette
  ...
  // Make it tall enough to cover the avatar head when you're on the wall tile
  const band  = 22;  // was 10/16. This is the "covers your head" lever.
  const inset = 3;
  const capH  = 6;

  // Build a set of player tiles (stable: NO rounding)
  const playerTileSet = new Set();
  const hh = state.holdingHands;

  const addPl = (pl) => {
    const tx = Math.floor(pl.fx);
    const ty = Math.floor(pl.fy);
    playerTileSet.add(`${tx},${ty}`);
  };
  ...
  if (hh) {
    ...
    for (const pl of state.players) addPl(pl);
  }

  const playerOnTile = (x, y) => playerTileSet.has(`${x},${y}`);

  // --- Helper: draw a horizontal wall between (x,y) and (x,y+1)
  // Render it mostly in the TOP tile.
  ...
  // Render it mostly in the LEFT tile.
  ...
  // PASS RULES:
  // - back pass: draw ALL walls so they're always visible
  // - front pass: redraw ONLY the wall tiles the player is standing on (so the wall covers you),
  //               but never covers the tile below (no clipping)
  const drawAll = (pass === "back");
    ...
    for (let y = 0; y < walls.h.length; y++) {
      ...
      for (let x = 0; x < row.length; x++) {
        ...
        if (drawAll) {
          ...
          if (playerOnTile(x, y)) drawWallH(x, y);
        }
      }
    }
  }
    ...
    for (let y = 0; y < walls.v.length; y++) {
      ...
      for (let x = 0; x < row.length; x++) {
        ...
        if (drawAll) {
          ...
          if (playerOnTile(x, y)) drawWallV(x, y);
        }
      }
    }
  }
...
if (state.mode === "overworld" && deferredLargeObjects.length) {
  ...
  if (hh) {
    ...
    for (let i = 0; i < state.players.length; i++) {
      ...
      // Clip avatar to tile if standing on a wall-owning tile
let clipped = false;
...
if (state.mode === "interior") {
  ...
  if (wallOnThisTile) {
    ctx.save();
    ctx.beginPath();
    ctx.rect(sx, sy, TILE_SIZE, TILE_SIZE);
    ctx.clip();
    clipped = true;
  }
}
