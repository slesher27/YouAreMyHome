...
if (isLeftWall) {
  ...
  drawRect(sx + inset, sy + inset, TILE_SIZE - inset * 2, TILE_SIZE - inset * 2, inner);
    ...
    // Interior-facing edge highlights (top + sides stay the same)
    if (y === 0)
      drawRect(sx + 4, sy + TILE_SIZE - 6, TILE_SIZE - 8, 2, "rgba(255,255,255,0.12)");
    if (x === 0)
      drawRect(sx + TILE_SIZE - 6, sy + 4, 2, TILE_SIZE - 8, "rgba(255,255,255,0.12)");
    if (x === INTERIOR_W - 1)
      drawRect(sx + 4, sy + 4, 2, TILE_SIZE - 8, "rgba(255,255,255,0.12)");

    // --- Bottom wall = exterior log wall ---
    if (isBottomWall && !doorHere) {
      ...
      const bandH = Math.floor((TILE_SIZE - 12) / 3);
      const insetX = 6;
      const startY = sy + 6;

      for (let i = 0; i < 3; i++) {
        const yy = startY + i * bandH;
        const alt = ((x + i) & 1) === 0;

        drawRect(sx + insetX, yy, TILE_SIZE - insetX * 2, bandH - 2, alt ? logA : logB);
        drawRect(sx + insetX, yy + 1, TILE_SIZE - insetX * 2, 1, hi);

        if (i < 2)
          drawRect(sx + 4, yy + bandH - 2, TILE_SIZE - 8, 2, chink);
      }
    } else if (isBottomWall) {
      // If it's bottom wall AND not log-painted (door area), keep a subtle inner lip
...
if (isWindowSpot && isWall) {
  ...
  if (doorHere) {
    ...
    // Cabin-ish window frame (matches log wall better than floating blue)
    drawRect(fx, fy, fw, fh, "rgba(255,255,255,0.25)");           // outer frame
    drawRect(fx + 2, fy + 2, fw - 4, fh - 4, "#4a79a8");          // glass
    drawRect(fx + 3, fy + 3, fw - 6, fh - 6, "rgba(255,255,255,0.18)");
