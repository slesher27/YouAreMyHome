...
function connectOnline(url) {
  ...
  ws.onmessage = (ev) => {
    ...
    if (msg.type === "welcome") {
      ...
      const wid = getActiveWorldId();
const payload = wid ? loadWorldPayload(wid) : null;

// If we have a saved world locally, ONLY seed it if it matches the expected grid shape.
// Otherwise regenerate so the server doesn't become authoritative over a broken payload.
if (wid && payload && isValidWorldPayload(payload)) {
  console.log("[NET] seeding server world from local save:", wid);
  sendNet({ type: "world_set", worldId: wid, world: payload });
  return;
}

if (wid && payload && !isValidWorldPayload(payload)) {
  console.warn("[NET] local saved world is invalid; regenerating and reseeding:", wid);
  state.world = generateOverworld();

  const p = worldToPayload(state.world);
  saveWorldPayload(wid, p);

  sendNet({ type: "world_set", worldId: wid, world: p });
  return;
}

      // No local saved payload:
      // - If host: create/generate a world and seed it so ops will broadcast
      ...
      if (state.net.isHost) {
        ...
        const newId = (typeof wid === "number") ? wid : Date.now();
        setActiveWorldId(newId);

        const p = worldToPayload(state.world);
        saveWorldPayload(newId, p);

        sendNet({ type: "world_set", worldId: newId, world: p });
      } else {
...
function setOverworldObject(x, y, value, broadcast = true) {
  ...
  // local apply
  state.net.suppressWorldOps = true;
state.world.objects[y][x] = value ?? null;
state.net.suppressWorldOps = false;

  // net broadcast
  if (broadcast && state.net?.enabled) {
    sendWorldOp({ kind: "set_obj", mode: "overworld", x, y, value: value ?? null });
  }
}
  ...
  for (let y = 0; y < map.objects.length; y++) {
    ...
    const proxyRow = new Proxy(row, {
      set(target, prop, value) {
        ...
        // Only broadcast numeric x assignments
        ...
        const x = key | 0;
        // bounds safety
        if (yy < 0 || yy >= WORLD_H || x < 0 || x >= WORLD_W) return true;

        sendWorldOp({ kind: "set_obj", mode: "overworld", x, y: yy, value: value ?? null });
        return true;
      }
    });
