...
const BUILD_STAMINA_COST_PER_SEC = 0.50;
...
function drawConstructionUI() {
  ...
  drawText("Build Site", rx + 10, ry + 18, 13);
  ...
  if (stageNeeds) {
    ...
    buildBtn = { x: rx + rightW - 150, y: ry + 12, w: 140, h: 26 };
  ...
  const progressX = rx + 120;
  const progressY = ry + 92;

  drawText(`Overall Progress: ${Math.round(pct * 100)}%`, progressX, progressY, 12, "left", "rgba(255,255,255,0.85)");

  const pbx = progressX;
  const pby = ry + 90;
  const pbw = Math.max(80, (rx + rightW) - pbx - 170); // leave room for build button
  const pbh = 10;
  ...
  for (const matId of needKeys.slice(0, 6)) {
    ...
    drawText(`${label}: ${remaining[matId]}`, needX, ny, 12, "left", "rgba(255,255,255,0.80)");
    ny += 16;
  }
  if (needKeys.length === 0) {
    drawText("Nothing. Ready to build.", needX, ny, 12, "left", "rgba(120,220,120,0.9)");
    ny += 16;
  }
  ...
  const logBox = { x: rx + 10, y: logTitleY + 18, w: rightW - 20, h: (ry + rh) - (logTitleY + 18) };
