...
function logAction(text) {
  ...
  // Only treat server as authoritative if the socket is actually open.
  ...
  if (canSend && !state.net.suppressLogs) {
    ...
    return; // wait for server echo to avoid duplicates
  }
...
// Apply a server log entry (dedupe by seq)
function applyNetLogEntry(entry) {
  if (!entry) return;

  // allow either {seq,text} or plain strings (if you ever send those)
