...
ws.onmessage = function (ev) {
  ...
  try {
    if (msg && msg.type === "snapshot") {
      var playerCount = 0;
      if (msg.state && msg.state.players && msg.state.players.length) playerCount = msg.state.players.length;
      ...
      console.log("[NET][RECV]", (msg ? msg.type : msg), msg);
    }
  } catch (e2) {
    console.error("[NET][RECV] trace failed:", e2);
  }
  ...
  try {
    ...
    if (msg.type === "welcome") {
      ...
      console.log("[NET] welcome:", { playerId: state.net.playerId, isHost: state.net.isHost });

      // If log_init arrived before welcome, try seeding now that host status is known.
      var serverCount = (state.net._lastServerLogCount != null) ? state.net._lastServerLogCount : 0;
    ...
    if (msg.type === "snapshot") {
      ...
      if (msg.state && Array.isArray(msg.state.players)) {
        if (typeof applyServerSnapshot === "function") {
          applyServerSnapshot(msg.state);
        } else {
          console.warn("[NET] applyServerSnapshot missing");
        }
      }
      ...
      if (Array.isArray(msg.ops) && msg.ops.length) {
        ...
        for (var i = 0; i < msg.ops.length; i++) {
          var op = msg.ops[i];
          if (typeof applyWorldOp === "function") {
            applyWorldOp(op);
            appliedOps++;
          }
        }
        console.log("[NET] snapshot ops applied:", appliedOps);
      }

      // logs
      if (Array.isArray(msg.logs)) {
        console.log("[NET] snapshot logs raw:", msg.logs);

        var appliedLogs = 0;
        for (var j = 0; j < msg.logs.length; j++) {
          var entry = msg.logs[j];
          if (typeof applyNetLogEntry === "function") {
            applyNetLogEntry(entry);
            appliedLogs++;
          }
        }
        console.log("[NET] snapshot logs applied:", appliedLogs);
    ...
    if (msg.type === "log_init") {
      ...
      if (Array.isArray(msg.entries)) {
        for (var k = 0; k < msg.entries.length; k++) {
          var e = msg.entries[k];
          var t = (e && typeof e === "object") ? e.text : e;
          if (t) incoming.push(String(t));
        }
        // newest-first
        incoming.reverse();
      }

      // merge (don’t clobber local)
      if (!Array.isArray(state.actionLog) || state.actionLog.length === 0) {
        state.actionLog = incoming;
      } else {
        var have = Object.create(null);
        for (var h = 0; h < state.actionLog.length; h++) have[state.actionLog[h]] = 1;

        for (var z = incoming.length - 1; z >= 0; z--) {
          var line = incoming[z];
          if (!have[line]) {
    ...
    if (msg.type === "log_entry") {
      ...
      if (typeof applyNetLogEntry === "function") applyNetLogEntry(msg.entry);
      if (typeof renderActivityLogDOM === "function") renderActivityLogDOM();
      return;
    }

    // WORLD APPLIED (optional server debug)
  ...
  ws.onerror = (e) => console.log("[NET] error:", e);
