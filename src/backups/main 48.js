...
if (state.mode === "interior") {
  ...
  // -------- VERTICAL (thin implied line + cap; no 3D) --------
  if (walls?.v?.[y]?.[x]) {
    // vertical edge between (x,y) and (x+1,y) → draw on RIGHT edge of this tile
    const thick = 2;
    const wallH = Math.floor(TILE_SIZE * 0.70);
    const bottom = sy + TILE_SIZE;
    const y0 = bottom - wallH;
    ...
    drawRect(lx, y0, thick, wallH, lineCol);

    const capW = 10;
    const capH2 = 3;
    const cx = lx + Math.floor(thick / 2) - Math.floor(capW / 2);

    drawRect(cx, y0 - capH2 + 1, capW, capH2, capCol);
    drawRect(cx, y0 - capH2 + 1, capW, 1, capHi);
    drawRect(cx, y0, capW, 1, capSh);
  }
}
