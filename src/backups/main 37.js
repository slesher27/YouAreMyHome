...
function drawConstructionUI() {
  ...
  if (stageNeeds) {
    ...
    buildBtn = { x: rx + rightW - 170, y: ry + 12, w: 130, h: 26 };
  ...
  const pbx = progressX;
  const pby = ry + 99;
  const pbw = Math.max(280, (rx + rightW) - pbx - 170); // leave room for build button
  const pbh = 8;
  ...
  for (const matId of needKeys.slice(0, 6)) {
    ...
    drawText(`${label}: ${remaining[matId]}`, needX, ny, 16, "left", "rgba(255,255,255,0.80)");
    ny += 16;
  }
  ...
  const logBox = { x: rx + 10, y: logTitleY + 13, w: rightW - 35, h: (ry + rh) - (logTitleY + 18) };
