...
function isVisibleNow(x, y) {
  ...
  // Night: within player sight OR campfire light
  ...
  : (isDay ? explored : (isInPlayerSight(x, y) || isLitByCampfire(x, y)));
      ...
      if (state.mode === "overworld") {
        ...
        if (!explored) {
          ...
          // Night: explored tiles are dark everywhere,
          // but campfire tiles are less dark (warm glow).
          const byFire = isLitByCampfire(x, y);
          const a = byFire ? CAMPFIRE_LIT_DARK_ALPHA : NIGHT_DARK_ALPHA;
          drawRect(sx, sy, TILE_SIZE, TILE_SIZE, `rgba(0,0,0,${a})`);
        }
      }
