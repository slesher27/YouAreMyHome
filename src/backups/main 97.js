...
function connectOnline(url) {
  ...
  ws.onopen = () => {
    ...
    ws.send(JSON.stringify({ type: "hello" })); // harmless
    ...
    // Do NOT clear state.actionLog here.
    // Server will send log_init, and we’ll merge it safely (next patch).
  };

  ws.onmessage = (ev) => {
    // Only log interesting messages (otherwise snapshot spam hides the real issue)
try {
  const _m = JSON.parse(ev.data);
  if (_m.type !== "snapshot" || (_m.ops && _m.ops.length)) {
  ...
  console.log("[NET] msg:", ev.data);
    ...
    // ------------------------------------------------------------
    // WELCOME: decide player slot + seed/request world
    // ------------------------------------------------------------
if (msg.type === "welcome") {
  state.net.playerId = msg.playerId;
  state.activePlayer = (msg.playerId === "p2") ? 1 : 0;

  // Host is always p1 (first connected client)
  state.net.isHost = (msg.playerId === "p1");

  // If log_init arrived before welcome, we now finally know host status.
  // Try seeding again (safe; will only seed if host + server empty).
  const serverCount = state.net._lastServerLogCount ?? 0;
  console.log("[NET][LOG] welcome -> host?", state.net.isHost, "serverCount?", serverCount, "localCount?", (state.actionLog?.length ?? 0));
  seedServerWithLocalLogIfNeeded(serverCount);

  // IMPORTANT: server only accepts numeric worldId
  const widRaw = getActiveWorldId();
  const widNum = (typeof widRaw === "number") ? widRaw : null;

  // Non-host MUST NOT seed. Ever. It should only request.
  if (!state.net.isHost) {
  ...
  // Host path: seed from local save (if valid) or generate and seed.
  const payloadRaw = widRaw ? loadWorldPayload(widRaw) : null;

if (
  payloadRaw &&
  (!payloadRaw.tiles || !payloadRaw.objects || !Array.isArray(payloadRaw.tiles) || !Array.isArray(payloadRaw.objects))
) {
    const seedId = widNum ?? Date.now();
    console.warn("[NET] host local saved world is invalid; regenerating and reseeding:", seedId);

    state.world = generateOverworld();
    const p = worldToPayload(state.world);

    setActiveWorldId(seedId);
    saveWorldPayload(seedId, p);

    sendNet({ type: "world_set", worldId: seedId, world: p });
    return;
  }

  // No local saved payload on host: generate + seed
  console.log("[NET] host has no saved world; generating + seeding a new one");
...
if (msg.type === "snapshot") {
  ...
  if (Array.isArray(msg.ops) && msg.ops.length) {
    ...
    state.net.suppressWorldOps = true;
    try {
      for (const op of msg.ops) applyWorldOp(op);
    } finally {
      state.net.suppressWorldOps = prev;
    }
    console.log("[NET] snapshot ops applied:", msg.ops.length);
  }

  // logs (independent of ops)
  if (Array.isArray(msg.logs) && msg.logs.length) {
    for (const e of msg.logs) applyNetLogEntry(e);
    console.log("[NET] snapshot logs applied:", msg.logs.length);
  }
...
// ------------------------------------------------------------
// SNAPSHOT (authoritative state + reliability piggyback)
// ------------------------------------------------------------
if (msg.type === "snapshot") {
  // players
  if (msg.state && Array.isArray(msg.state.players)) {
    applyServerSnapshot(msg.state);
  }

  // world ops (catch up anyone who missed real-time ops)
  if (Array.isArray(msg.ops) && msg.ops.length) {
    for (const op of msg.ops) applyWorldOp(op);
  }

  // logs (catch up anyone who missed log_entry/log_init)
  if (Array.isArray(msg.logs) && msg.logs.length) {
    for (const entry of msg.logs) applyNetLogEntry(entry);
  }
...
// ------------------------------------------------------------
// LOG INIT (full sync on connect)
// ------------------------------------------------------------
if (msg.type === "log_init") {
  state.net.lastLogSeq = (msg.seq | 0);

  // Remember what the server had so we can seed later if welcome arrives after this.
  state.net._lastServerLogCount = Array.isArray(msg.entries) ? msg.entries.length : 0;
  console.log("[NET][LOG] log_init received. serverCount =", state.net._lastServerLogCount);

  const incoming = Array.isArray(msg.entries)
    ? msg.entries
        .map(e => (typeof e === "object" && e ? e.text : e))
        .filter(Boolean)
        .reverse() // newest first
    : [];

seedServerWithLocalLogIfNeeded(incoming.length);

  // Merge instead of clobbering, so locally-created (buffered) lines survive.
  if (!Array.isArray(state.actionLog) || state.actionLog.length === 0) {
    state.actionLog = incoming;
  } else {
    const have = new Set(state.actionLog);
    // Add missing server lines as older history (append at end)
  ...
  // If anything got buffered after open but before init arrived, flush it.
  flushBufferedLogs();

  console.log("[NET] log_init", {
    seq: state.net.lastLogSeq,
    entries: state.actionLog.length
  });
...
if (msg.type === "log_entry") {
  console.log("[NET] log_entry recv", msg.entry);
  ...
  return;
    ...
    if (msg.type === "world") {
      if (typeof msg.worldId === "number" && msg.world) {
        setActiveWorldId(msg.worldId);
        saveWorldPayload(msg.worldId, msg.world);

        state.world = payloadToWorld(msg.world);
		enableOverworldObjectsProxy();

        // clear paths so nothing tries to walk in old terrain
    ...
    // ------------------------------------------------------------
    // WORLD OPS (tree chops, pickups, etc.)
    // ------------------------------------------------------------
if (msg.type === "world_op") {
  console.log("[NET] recv world_op <-", msg.op);
  applyWorldOp(msg.op);
  return;
}
  };
...
function setOverworldObject(x, y, value, broadcast = true) {
  ...
  state.net.suppressWorldOps = true;
  state.world.objects[y][x] = nextNorm;
  state.net.suppressWorldOps = false;

  if (broadcast && state.net?.enabled) {
    sendWorldOp({ kind: "set_obj", mode: "overworld", x, y, value: nextNorm });
  }
}
...
function updateAnimals(dt) {
	if (state.net?.enabled) return; // TEMP: don't move animals while online
...
// Remove harvested object (network-synced)
setOverworldObject(x, y, null);

  // Damage / remove object (run ONCE per harvest, not per item)
  obj.hp = (obj.hp ?? 1) - 1;

  // Helper: in overworld, commit via net-synced setter
  const commitObj = (value) => {
  if (state.mode === "overworld") {
    setOverworldObject(x, y, value ?? null);
  } else {
    // fallback only for non-networked maps
    setOverworldObject(x, y, null);
  }
  return true;
};
