...
music: {
  ...
  switching: false,
  pendingCtxKey: null,
  indexMax: 10         // how many numbered tracks to try (you can raise this)
},
...
state.title.wantTutorial = !state.tutorial.seen; // unchecked after you've seen it
...
function drawBottomRightToolbar() {
  // anchor near bottom-right, just above the bottom UI bar
 const pad = 10;
const btn = { w: 36, h: 30 };

// hard bottom-right anchor
const x = window.innerWidth - pad - btn.w;
const y = window.innerHeight - pad - btn.h;

  // save hitboxes
  state._brToolbarUI = {
    musicBtn: { x, y, w: btn.w, h: btn.h },
    sliderBox: null,
    sliderTrack: null,
    sliderKnob: null
  };
  ...
  // button
  drawRect(x, y, btn.w, btn.h, "rgba(0,0,0,0.55)");
  ctx.strokeStyle = "rgba(255,255,255,0.18)";
  ctx.strokeRect(x, y, btn.w, btn.h);
  drawText("🔊", x + btn.w / 2, y + btn.h / 2 + 1, 16, "center");
  ...
  const boxW = 220;
  const boxH = 58;
  const bx = x - boxW + btn.w;         // pop left from icon
  const by = y - boxH - 6;             // pop upward
  ...
  state._brToolbarUI.sliderBox = { x: bx, y: by, w: boxW, h: boxH };
  state._brToolbarUI.sliderTrack = track;
  state._brToolbarUI.sliderKnob = knob;
}
...
function handleBottomRightToolbarTap(px, py) {
  const ui = state._brToolbarUI;
  if (!ui) return false;

  // Clicking the icon toggles the slider
  if (hitRect(px, py, ui.musicBtn)) {
  ...
  if (state.ui.musicSliderOpen) {
    // Click inside slider box but not on track? keep it open
    if (ui.sliderBox && hitRect(px, py, ui.sliderBox)) {
      // click/drag on track sets volume
...
function handleBottomRightToolbarDrag(px, py) {
...
const TUTORIAL_SLIDES = [
  ...
  { title: "Stamina ⚡️", text: "Moving and certain other actions cost stamina. You can rest at any time to recover stamina! Just tap or click on your character to access the character menu, and choose rest! On a keyboard, you can use the R key." },
  ...
  { title: "Construction 🔨", text: "Once you've obtained a complete blueprint and the proper resources, you can build and customize a house!" },
  ...
  { title: "Locations 🧭", text: "If you ever feel lost, you can always access the Location Markers. Certain map objects will automatically be marked. Additionally, you can manually mark any location on the map!" },
...
function drawTitleMenu() {
  ...
  // checkmark (CENTERED in the box)
  if (!state.tutorial.seen && state.title.wantTutorial !== false) {
...
function handleTitleMenuTap(px, py) {
  ...
  if (ui.tutorialToggle && hitRect(px, py, ui.tutorialToggle)) {
    // If they've already completed it once, keep it forced off
    if (!state.tutorial?.seen) {
      state.title.wantTutorial = !state.title.wantTutorial;
    } else {
      state.title.wantTutorial = false;
    }
    return true;
  }
  ...
  if (ui.startBtn && hitRect(px, py, ui.startBtn)) {
    ...
    // If tutorial is enabled and not yet seen, open tutorial instead of starting immediately
    if (!state.tutorial?.seen && state.title.wantTutorial !== false) {
...
canvas.addEventListener("pointerdown", (e) => {
  ...
  // Bottom-right toolbar (volume slider)
  if (handleBottomRightToolbarTap(px, py)) return;
...
// dragging the slider
if (handleBottomRightToolbarDrag(px, py)) return;
...
if (state.tutorial?.open) {
  ...
  if (state.tutorial.index >= TUTORIAL_SLIDES.length) {
    ...
    if (state.title) state.title.wantTutorial = false;
