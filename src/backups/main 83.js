...
// ------------------------------------------------------------
// PLAYER SNAPSHOTS (+ piggybacked world ops)
// ------------------------------------------------------------
if (msg.type === "snapshot") {
  applyServerSnapshot(msg.state);
  state.net.lastSnapshotAt = performance.now();

  // ✅ Apply queued world ops that arrived with the snapshot
  ...
  // ✅ Apply queued log entries piggybacked on snapshot (MUST be independent of ops)
...
if (msg.type === "log_init") {
  ...
  // Replace local log with server log
  if (Array.isArray(msg.entries)) {
    state.actionLog = msg.entries.map(e => (typeof e === "object" ? e.text : e)).filter(Boolean);
    // server list is oldest..newest; your UI expects newest first
...
function renderActivityLogDOM() {
  const dom = ensureActivityLogDOM();
  if (!dom) return;

  const { body } = dom;

  // state.actionLog is newest-first in your code
  const rows = (state.actionLog || []).slice(0, 40);

  body.innerHTML = rows
    .map(t => `<div style="margin:0 0 4px 0; opacity:.95;">• ${escapeHtml(String(t))}</div>`)
    .join("");

  // Keep view pinned to newest
  body.scrollTop = 0;
}
