...
loadSound("footstepsindoor", "footsteps_indoor.mp3", 0.5);
loadSound("footstepsoutdoor1", "footsteps_outdoor1.mp3", 0.5);
loadSound("footstepsoutdoor2", "footsteps_outdoor2.mp3", 0.5);
loadSound("footstepsoutdoor3", "footsteps_outdoor3.mp3", 0.5);
...
function openMenuForTile(mapX, mapY, screenX, screenY) {
  ...
  const hasBow = getQty(activeInv(), "bow_and_arrow") > 0;
  ...
  if (interact) {
    ...
    options.push({
      label: isHuntable ? (dist <= 1 ? "Hunt" : "Shoot") : "Harvest resource",
      disabledReason,
      action: () => {
        if (disabledReason) return;
  ...
  if (["bush", "berry_bush", "apple_tree"].includes(obj.id) && hasTool(inv, "axe")) {
    options.push({
      label: "Chop down",
      action: () => {
  ...
  options.push({ label, action: () => openContainerObject(mapX, mapY) });
}
...
if (obj.meta?.depleted) {
  drawRect(sx, sy, TILE_SIZE, TILE_SIZE, "rgba(0,0,0,0.25)");
  drawText("depleted", sx + TILE_SIZE/2, sy + TILE_SIZE - 10, 11, "center", "rgba(255,255,255,0.75)");
}
