...
function drawBuiltInteriorWallsPass(pass) {
  ...
  const band = 10;        // overall wall thickness
  const inset = 2;        // inner inset
  const capH = 4;         // small cap strip for the built wall
...
function generateInterior(interiorId) {
  ...
  // If saved layout exists, apply it ON TOP of the template
...
function placeInteriorObjectAt(x, y, objId, wTiles) {
  ...
  clearInteriorPlacedObject(x, y);

  // Place anchor object with metadata
  map.objects[y][x] = { id: objId, hp: 999, meta: { wTiles } };

  // If multi-tile (bed), mark the extra tiles as occupied proxies
  for (let dx = 1; dx < wTiles; dx++) {
