...
function connectOnline(url) {
  ...
  ws.onopen = () => {
    ...
    // server doesn't actually need "hello", but harmless
  ...
  ws.onmessage = (ev) => {
    ...
    if (msg.type === "welcome") {
      ...
      // If we have a saved world locally, seed it to the server.
      // (Server accepts world_set from either client, and this prevents "world_op ignored".)
      if (wid && payload) {
