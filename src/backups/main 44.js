...
if (state.mode === "interior") {
  ...
  if (walls) {
    ...
    if (walls.h?.[y]?.[x]) {
      const by = sy + TILE_SIZE;     // boundary between (y) and (y+1)
      const y0 = by - band;          // wall body pushed into this tile
    ...
    if (walls.v?.[y]?.[x]) {
      const bx = sx + TILE_SIZE;     // boundary between (x) and (x+1)
      const x0 = bx - band;          // wall body pushed into this tile
