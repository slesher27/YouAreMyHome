...
function drawBuiltInteriorWallsPass(pass) {
  ...
  // Try to match your existing wall palette (tweak if needed to match exactly)
  const logA = "#c8923b";
  const logB = "#b47c2f";
  const chink = "rgba(40,20,8,0.55)";
  const hi = "rgba(255,255,255,0.10)";

  // Thickness of the "real wall" band that overlaps both tiles
  const BAND = 12;          // overall wall thickness
  const LIP = 4;            // occluding front lip thickness
  const INSET = 3;          // inset margins so it feels like your existing walls

  // BACK pass draws the full chunky wall body.
  // FRONT pass draws only a small interior-facing lip to cover the player.
  const drawBody = (pass === "back");
  const drawLip  = (pass === "front");

  // Horizontal walls: map.walls.h[y][x] blocks between (x,y) and (x,y+1)
  // We draw centered on the boundary between tiles y and y+1, overlapping both.
  for (let y = 0; y < INTERIOR_H - 1; y++) {
    for (let x = 0; x < INTERIOR_W; x++) {
      if (!map.walls.h?.[y]?.[x]) continue;

      const sx = UI_LEFTBAR_W + x * TILE_SIZE;
      const sy = y * TILE_SIZE;

      // boundary is at sy + TILE_SIZE
      const by = sy + TILE_SIZE;

      if (drawBody) {
        // Main log band
        drawRect(sx + 2, by - Math.floor(BAND / 2), TILE_SIZE - 4, BAND, logA);

        // Log segments + chinking lines (gives that "existing wall" vibe)
        for (let k = 0; k < 3; k++) {
          const yy = (by - Math.floor(BAND / 2)) + 2 + k * 4;
          drawRect(sx + 4, yy, TILE_SIZE - 8, 2, (k % 2 === 0) ? chink : "rgba(0,0,0,0.12)");
        }

        // Highlights (subtle)
        drawRect(sx + 4, by - Math.floor(BAND / 2) + 2, TILE_SIZE - 8, 2, hi);
        drawRect(sx + 4, by + Math.floor(BAND / 2) - 4, TILE_SIZE - 8, 2, "rgba(0,0,0,0.15)");
      }

      if (drawLip) {
        // Interior-facing lip: pick which side is "interior facing"
        // For a horizontal wall, interior-facing could be either top or bottom.
        // We'll make the lip sit slightly "above" the boundary so it covers
        // a character on the far side consistently (matches your screenshot feel).
        drawRect(sx + INSET, by - Math.floor(LIP / 2) - 2, TILE_SIZE - INSET * 2, LIP, "rgba(30,15,6,0.35)");
      }
    }
  }

  // Vertical walls: map.walls.v[y][x] blocks between (x,y) and (x+1,y)
  // boundary is at x+1 tile's left edge
  for (let y = 0; y < INTERIOR_H; y++) {
    for (let x = 0; x < INTERIOR_W - 1; x++) {
      if (!map.walls.v?.[y]?.[x]) continue;

      const sx = UI_LEFTBAR_W + x * TILE_SIZE;
      const sy = y * TILE_SIZE;

      // boundary is at sx + TILE_SIZE
      const bx = sx + TILE_SIZE;

      if (drawBody) {
        // Main vertical log band
        drawRect(bx - Math.floor(BAND / 2), sy + 2, BAND, TILE_SIZE - 4, logA);

        // vertical chink lines
        for (let k = 0; k < 3; k++) {
          const xx = (bx - Math.floor(BAND / 2)) + 2 + k * 4;
          drawRect(xx, sy + 4, 2, TILE_SIZE - 8, (k % 2 === 0) ? chink : "rgba(0,0,0,0.12)");
        }

        // highlights
        drawRect(bx - Math.floor(BAND / 2) + 2, sy + 4, 2, TILE_SIZE - 8, hi);
        drawRect(bx + Math.floor(BAND / 2) - 4, sy + 4, 2, TILE_SIZE - 8, "rgba(0,0,0,0.15)");
      }

      if (drawLip) {
        // Interior-facing lip for vertical wall: place slightly left of boundary
        drawRect(bx - Math.floor(LIP / 2) - 2, sy + INSET, LIP, TILE_SIZE - INSET * 2, "rgba(30,15,6,0.35)");
      }
    }
  }
}
...
drawBuiltInteriorWallsPass("back");
...
if (o?.icon) {
  ...
  if (state.mode === "interior") {
    ...
    if (obj.id === "occupied" && obj.meta?.anchor) {
      ...
      drawIconFitWidth(o.icon, sx, sy, targetW);
    }
  } else {
    // Overworld: keep your centered behavior
...
drawBuiltInteriorWallsPass("front");
}
