...
function updateCampfires(dt) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      obj.meta = obj.meta || {};
      if ((obj.meta.fireT ?? 0) <= 0) {
        obj.meta.fireT = 0;
        continue;
      }

      obj.meta.fireT = Math.max(0, (obj.meta.fireT ?? 0) - dt);
    }
  }
}
...
function isVisibleNow(x, y) {
  ...
  return isInPlayerSight(x, y) || isLitByCampfire(x, y);
}
    ...
    for (let x = camX; x < Math.min(w, camX + viewW); x++) {
      ...
      if (state.mode === "overworld") {
        ...
        if (!explored) {
          ...
          if (!isDay && !visNow) {
            drawRect(sx, sy, TILE_SIZE, TILE_SIZE, "rgba(0,0,0,0.55)");
          }
        }
      }
    }
...
function updateTreeRegrowthOnNewDay() {
  ...
  for (let y = 0; y < WORLD_H; y++) {
    for (let x = 0; x < WORLD_W; x++) {
      const obj = map.objects[y][x];
      if (!obj?.meta?.regrowTimer) continue;

      // Prevent same-day aging
      if (obj.meta.bornDay === today) continue;

      obj.meta.regrowTimer--;

      // stump → seedling
      if (obj.id === "stump" && obj.meta.regrowTimer <= 0) {
        map.objects[y][x] = {
          id: "stump_seedling",
          meta: {
            originalTree: obj.meta.originalTree,
            regrowTimer: 3,
            bornDay: today
          }
        };
      ...
      // seedling → tree
      else if (obj.id === "stump_seedling" && obj.meta.regrowTimer <= 0) {
        map.objects[y][x] = {
          id: obj.meta.originalTree
        };
      }
    }
  }
