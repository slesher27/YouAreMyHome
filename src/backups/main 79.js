...
function logAction(text) {
  // If we're online, treat the server as authoritative for ordering.
  // So: send log request and WAIT for server to echo it back (avoids duplicates).
  if (state.net?.enabled && !state.net.suppressLogs) {
    sendNet({ type: "input", payload: { type: "log", text: String(text ?? "") } });
    return;
  }

  // Offline (or applying server log): local append
  state.actionLog.unshift(String(text ?? ""));
  if (state.actionLog.length > 200) state.actionLog.pop();

  // Keep DOM panel in sync
