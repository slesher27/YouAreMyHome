...
function sendWorldOp(op) {
  ...
  if (state.net.suppressWorldOps) return; // never echo while applying remote ops
...
function applyWorldOp(op) {
  ...
  // If overworld objects proxy is installed, applying an op will mutate map.objects,
  // which can trigger the proxy setter and re-send the op forever.
  // So we suppress outbound world ops during application.
  const hadNet = !!state.net;
  const prevSuppress = hadNet ? !!state.net.suppressWorldOps : false;
  if (hadNet) state.net.suppressWorldOps = true;

  try {
    // For now, overworld objects only.
    if (op.kind === "set_obj") {
    ...
    if (hadNet) state.net.suppressWorldOps = prevSuppress;
  }
}
