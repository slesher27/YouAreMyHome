...
function harvestObject(x, y) {
  ...
  for (const [itemId, amt] of Object.entries(interact.gives || {})) {
    ...
    else if (def.hunt) logAction(`${pName} hunted ${amt} ${itemName}${amt === 1 ? "" : "s"}!`);
    else logAction(`${pName} harvested ${amt} ${itemName}${amt === 1 ? "" : "s"}!`);
  }

  // Damage / remove object (animals should disappear after 1 hunt unless you set hp higher)
obj.hp = (obj.hp ?? 1) - 1;
...
function openMenuForTile(mapX, mapY, screenX, screenY) {
  ...
  // ---- Chop down blockers (anti-softlock) ----
  const choppable = new Set(["bush", "berry_bush", "apple_tree"]);
  if (choppable.has(obj.id)) {
