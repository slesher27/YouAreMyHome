...
loadSound("chop", "chop_wood.MP3", 0.3);
...
function attemptManualCraft() {
  ...
  // Find a recipe whose inputs match selected EXACTLY (ids + quantities)
  const r = recipeList().find(r => {
    ...
    for (const id of inIds) {
      if ((sel.get(id) ?? 0) !== ins[id]) return false;
    }
    return true;
  });
  ...
  // Station gating
  if (state.craftingMode === "craft" && r.station !== "workbench") return;
  if (state.craftingMode === "cook" && r.station !== "campfire") return;
...
canvas.addEventListener("pointerup", (e) => {
  ...
  // If you have other world-click behavior below this in your original file
  // (like pathfinding), it should continue here.
});

// Right click (inventory context menu) – MUST be outside pointerup
