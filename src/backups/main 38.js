...
const editBtn = (state.mode === "interior")
  ? mkBtn(settingsBtn.y + settingsBtn.h + gap)
  : null;
...
function openInteriorPlaceObjectMenu(tx, ty, px, py) {
  // Starter list. Replace/expand once we pick “household objects”.
  const candidates = [
    "chest",
    "toolbox",
    "workbench",
  ...
  const existing = state.interior?.objects?.[ty]?.[tx] ?? null;
  ...
  if (existing) {
    ...
    opts.push({
      ...
      action: () => {
        playSound("pop");
        state.interior.objects[ty][tx] = null;
        closeMenu();
      }
    });
  }
  ...
  if (candidates.length === 0) {
    opts.push({
      label: "(No household objects found yet)",
      disabledReason: "Add object ids to the list in openInteriorPlaceObjectMenu()",
      action: () => {}
    });
  } else {
    for (const objId of candidates) {
      ...
      opts.push({
        ...
        action: () => {
          playSound("pop");
          state.interior.objects[ty][tx] = { id: objId, hp: 999, meta: {} };
          closeMenu();
        }
      });
    }
...
if (state.menu) {
  ...
  if (state.mode === "interior" && state.interiorEdit?.on) {
...
if (k === "e") {
  ...
  if (!st) return;
    ...
    // DEV: Shift+I = enter interior instantly (testing)
  if (k === "i" && e.shiftKey) {
    e.preventDefault();
    ...
    if (state.mode !== "interior") {
      ...
      // optional: Shift+I while inside = force exit (no door check)
      state.mode = "overworld";
      closeMenu();
  ...
  playSound("pickup"); // change if you want a different sfx
