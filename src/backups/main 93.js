...
if (msg.type === "log_init") {
  ...
  // ✅ Ensure UI exists, then render once
  ensureActivityLogDOM();
...
function renderActivityLogDOM() {
  // Ensure the DOM exists (creates it only if missing)
  const ui = ensureActivityLogDOM();
  const body = ui?.body;
  if (!body) return;

  // newest-first, keep it short so it doesn't become a novel
