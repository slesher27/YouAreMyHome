...
function seedServerWithLocalLogIfNeeded(serverIncomingCount) {
  ...
  // Only the host should seed the server
  if (!state.net.isHost) return;

  // Only seed if server has nothing but we do
  if ((serverIncomingCount | 0) !== 0) return;
  if (!Array.isArray(state.actionLog) || state.actionLog.length === 0) return;
  ...
  console.log("[NET] seeded server log from host local history:", state.actionLog.length);
}
