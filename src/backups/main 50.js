...
for (let y = 1; y < WORLD_H - 1; y++) {
  ...
  placeObjects(map, DUNGEON_W, DUNGEON_H, "rat1", ["floor"], 8);
  placeObjects(map, DUNGEON_W, DUNGEON_H, "rat2", ["floor"], 8);
  ...
  placeObjects(map, DUNGEON_W, DUNGEON_H, "stone", ["floor"], 25);
  placeObjects(map, DUNGEON_W, DUNGEON_H, "wild_mushrooms", ["floor"], 10);
...
function generateDungeon(dungeonId, entrance) {
  ...
  for (let i = 0; i < chambers.length - 1; i++) {
    ...
    const path = carveWigglyCorridor(map, ax, ay, bx, by);
    ...
    for (let k = start; k <= end; k++) {
      const [gx, gy] = path[k];
      if (pointInRect(gx, gy, a) || pointInRect(gx, gy, b)) continue;
      gate = { x: gx, y: gy };
      break;
    }
    if (!gate) {
    ...
    if (gate) {
      map.objects[gate.y][gate.x] = { id: "dungeon_gate", hp: 999, meta: { locked: true, gateIndex: i } };
    }
  }
