...
function harvestObject(x, y) {
  ...
  if (baseChance < 1) {
    ...
    if (!chk.ok) {
      if (obj.id === "fish_spot") logAction(`${pName} tried to fish… nothing. (d${chk.sides}: ${chk.roll} > ${chk.target})`);
      else if (def.hunt) logAction(`${pName} tried to hunt… failed. (d${chk.sides}: ${chk.roll} > ${chk.target})`);
      else logAction(`${pName} tried to harvest… failed. (d${chk.sides}: ${chk.roll} > ${chk.target})`);
      closeMenu();
      return;
    }
  }
  ...
  for (const [itemId, amt] of Object.entries(interact.gives || {})) {
    addItemOrDrop(inv, itemId, amt, x, y, pName);
  }

  // ---- DAMAGE / UPDATE OBJECT (NETWORK SAFE) ----
  const curHp = (typeof obj.hp === "number") ? obj.hp : 1;
  const nextHp = curHp - 1;

  // Helper: only overworld uses net-synced setter
  const commit = (value) => {
    if (state.mode === "overworld") setOverworldObject(x, y, value ?? null);
    else map.objects[y][x] = value ?? null;
  };

  if (nextHp <= 0) {
    // Trees become stump (regrowth system)
    if (/^tree[1-8]$/.test(obj.id)) {
      commit({
        id: "stump",
        meta: {
    ...
    // Renewable resources become depleted (stay on map)
  ...
  // Still alive: update HP by REPLACING the cell object (no in-place mutation)
  commit({
    ...obj,
    hp: nextHp,
