...
for (let y = 1; y < WORLD_H - 1; y++) {
  ...
  // resources
  placeObjects(map, WORLD_W, WORLD_H, "tree", ["grass"], 70);
  ...
  // starter workbench
  map.objects[5][5] = { id: "workbench", hp: 999, meta: {} };
...
function chestLoot(inv) {
  ...
  const tools = ["axe", "hammer", "pickaxe", "fishing_pole"];
...
function openMenuForPlayer(plIndex, screenX, screenY) {
  ...
  openMenu({ screenX, screenY, title: `${pl.name}`, options });
}
...
function canCraftRecipe(r) {
  ...
  if (r.station) {
    ...
    for (const [nx, ny] of neighbors4(p.x, p.y)) {
      ...
      if (odef?.station === r.station) ok = true;
    }
...
const OBJECT_CURSORS = {
  tree: { icon: "axe.png", hotX: 10, hotY: 10 },
  rock: { icon: "pick.png", hotX: 10, hotY: 10 },
  chest: { icon: "key.png", hotX: 8, hotY: 4 }
};
