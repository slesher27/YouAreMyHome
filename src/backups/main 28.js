...
function drawLeftSidebar() {
  ...
  // Button background
  drawRect(btn.x, btn.y, btn.w, btn.h, "rgba(20,20,20,0.92)");
  ctx.strokeStyle = "rgba(255,255,255,0.16)";
  ctx.strokeRect(btn.x, btn.y, btn.w, btn.h);

  // Icon
  drawText("🧭", btn.x + btn.w / 2, btn.y + btn.h / 2 + 8, 22, "center", "rgba(255,255,255,0.95)");

  // Save hitbox for clicking
  state._leftbarUI = { compassBtn: btn };
}
    ...
    openMenu({
      ...
      options: [
        ...
        ...(typeof itemDef(itemId)?.nourishment === "number" ? [{
          ...
          action: withSfx("pickup", () => { // swap sfx id if you want a dedicated "eat" sound
            if (tooFar) return;
...
function drawTopBar() {
  ...
  // hunger bar (new) - placed to the right of stamina
  const staminaX = 70, staminaY = 34, staminaW = 160, staminaH = 12;

  const hungerX = staminaX + staminaW + 150; // shove it right so it doesn't clash with "Resting…"
  ...
  // put status text AFTER the hunger bar so it never overlaps
...
if (handleLeftSidebarTap(px, py)) { state.pointer.blocked = false; return; }
