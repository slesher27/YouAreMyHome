...
function renderActivityLogDOM() {
  // Try the expected IDs first, then common alternates
  const body =
    document.getElementById("activityLogBody") ||
    document.getElementById("actionLogBody") ||
    document.querySelector("#activityLogBody") ||
    document.querySelector("#actionLogBody");

  const panel =
    document.getElementById("activityLog") ||
    document.getElementById("actionLog") ||
    document.querySelector("#activityLog") ||
    document.querySelector("#actionLog");

  // If the DOM isn't there, don't silently pretend everything is fine.
  if (!body || !panel) {
    // one-time warning (so it doesn't spam)
    if (!renderActivityLogDOM._warned) {
      renderActivityLogDOM._warned = true;
      console.warn(
        "[LOG] Activity log DOM not found. Expected #activityLog/#activityLogBody (or #actionLog/#actionLogBody)."
      );
    }
    return;
  }
