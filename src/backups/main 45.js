...
window.deventerior = function (interiorId = "house_small") {
  ...
  // Build/reset interior map
  state.interior = generateInterior(interiorId);

  // Switch mode
  state.mode = "interior";

  // Put players near bottom
  ...
  // Try to add to inventory, otherwise drop at player feet
...
if (state.title.savedProfile === "scott") state.activePlayer = 0;
...
function getMusicContextKey() {
  // Title screen has its own music context
...
function ensureMusicAudio() {
  ...
  a.addEventListener("ended", () => {
    // when a track ends, pick another in the same context
...
function drawBuiltInteriorWallsPass(pass) {
  ...
  // Match your existing interior palette (from drawWorld interior branch)
  const wallOuter = "#3e2a17";
  const wallInner = "#523621";

  // Match your existing cap vibe (same constants you use on the border walls)
  const capWood = "rgba(180,120,60,0.60)";
  const capHi   = "rgba(255,255,255,0.18)";
  const capSh   = "rgba(0,0,0,0.30)";

  // Built-wall thickness: thinner than full wall tiles, but still readable
  const band = 16;        // overall wall thickness
  const inset = 3;        // inner inset
  const capH = 5;         // small cap strip for the built wall

  // --- Helper: draw a horizontal wall between (x,y) and (x,y+1)
  // We render it mostly in the TOP tile (so it "occupies the space above"),
  // which makes it naturally occlude characters on the north side when drawn in front-pass.
  function drawWallH(x, y) {
    const a = mapToScreen(x, y);           // top tile
    const by = a.sy + TILE_SIZE;           // boundary between y and y+1
    const y0 = by - band;                  // push into the tile ABOVE the boundary

    // body
    drawRect(a.sx, y0, TILE_SIZE, band, wallOuter);
    drawRect(a.sx + inset, y0 + inset, TILE_SIZE - inset * 2, band - inset * 2, wallInner);

    // cap strip (gives the same “finished edge” feel)
    drawRect(a.sx, y0, TILE_SIZE, capH, capWood);
    drawRect(a.sx, y0, TILE_SIZE, 1, capHi);
    drawRect(a.sx, y0 + capH - 1, TILE_SIZE, 1, capSh);
  }

  // --- Helper: draw a vertical wall between (x,y) and (x+1,y)
  // Render it mostly in the LEFT tile (so it “occupies the space to the left”),
  // keeping the perspective consistent with your half-thickness side walls.
  function drawWallV(x, y) {
    const a = mapToScreen(x, y);           // left tile
    const bx = a.sx + TILE_SIZE;           // boundary between x and x+1
    const x0 = bx - band;                  // push into the tile LEFT of boundary

    // body
    drawRect(x0, a.sy, band, TILE_SIZE, wallOuter);
    drawRect(x0 + inset, a.sy + inset, band - inset * 2, TILE_SIZE - inset * 2, wallInner);

    // cap strip (vertical)
    drawRect(x0, a.sy, capH, TILE_SIZE, capWood);
    drawRect(x0, a.sy, 1, TILE_SIZE, capHi);
    drawRect(x0 + capH - 1, a.sy, 1, TILE_SIZE, capSh);
  }

  // We split into passes, but we don’t need fancy depth math yet:
  // - back: draw only the “far” edges (subtle), so it doesn’t fight objects
  // - front: draw the full wall so it can occlude players cleanly
  const drawFull = (pass === "front");

  // Horizontal walls: walls.h[y][x] blocks between (x,y) and (x,y+1)
  if (walls.h) {
    for (let y = 0; y < walls.h.length; y++) {
      const row = walls.h[y];
      if (!row) continue;
      for (let x = 0; x < row.length; x++) {
        ...
        if (drawFull) {
          ...
          // back-pass hint (tiny shadow line at the boundary)
          const a = mapToScreen(x, y);
          const by = a.sy + TILE_SIZE;
          drawRect(a.sx, by - 2, TILE_SIZE, 2, "rgba(0,0,0,0.20)");
        }
      }
    }
  }

  // Vertical walls: walls.v[y][x] blocks between (x,y) and (x+1,y)
  if (walls.v) {
    for (let y = 0; y < walls.v.length; y++) {
      const row = walls.v[y];
      if (!row) continue;
      for (let x = 0; x < row.length; x++) {
        if (!row[x]) continue;
        if (drawFull) {
          ...
          // back-pass hint (tiny shadow line at the boundary)
          const a = mapToScreen(x, y);
          const bx = a.sx + TILE_SIZE;
          drawRect(bx - 2, a.sy, 2, TILE_SIZE, "rgba(0,0,0,0.20)");
        }
      }
    }
  }
  ...
  if (def?.fixedLayout && def?.layout) {
    ...
    if (Array.isArray(def.layout.objects)) {
      for (const o of def.layout.objects) {
        ...
        stampInteriorObject(map, x, y, id, wTiles, meta ?? null);
      }
    }
...
function isPassable(x, y) {
  ...
  // ✅ INTERIOR: wall tile should never be passable (door stays passable)
 ...
 const householdIds = Object.entries(state.data?.objects ?? {})
  ...
  if (householdIds.length === 0) {
    ...
    for (const objId of householdIds) {
      ...
      let disabledReason = null;
      if (!canPlaceInteriorObjectAt(anchor.x, anchor.y, wTiles)) {
      ...
      opts.push({
        ...
        action: () => {
          if (disabledReason) return;
          playSound("pop");
          placeInteriorObjectAt(anchor.x, anchor.y, objId, wTiles);
          closeMenu();
        }
      });
    }
...
function canPlaceInteriorObjectAt(x, y, wTiles) {
  const map = state.interior;
  const h = map?.tiles?.length ?? 0;
  const w = map?.tiles?.[0]?.length ?? 0;
  ...
  for (let dx = 0; dx < wTiles; dx++) {
    const t = map.objects?.[y]?.[x + dx] ?? null;
    if (!t) continue;

    // If it's an occupied proxy, treat it as blocked
    if (t.id === "occupied") return false;

    // Any real object blocks
    return false;
  }
  return true;
}
...
function clearInteriorPlacedObject(x, y) {
  ...
  // Clear any occupied tiles owned by this anchor
  const wTiles = obj.meta?.wTiles ?? 1;
  for (let dx = 1; dx < wTiles; dx++) {
    if (map.objects?.[y]?.[x + dx]?.id === "occupied") {
      map.objects[y][x + dx] = null;
    }
  }
    saveInteriorLayout();
}

function stampInteriorObject(map, x, y, objId, wTiles = 1, metaExtra = null) {
  if (!map?.objects?.[y]) return false;

  // Clear anything currently here (anchor clear)
  // (uses your existing clear logic, but it currently targets state.interior,
  // so we do a simple local clear for safety)
  map.objects[y][x] = null;
  for (let dx = 1; dx < Math.max(1, wTiles); dx++) {
    if (map.objects?.[y]?.[x + dx]) map.objects[y][x + dx] = null;
  }

  const meta = { wTiles, ...(metaExtra || {}) };

  // Anchor
  map.objects[y][x] = { id: objId, hp: 999, meta };

  // Occupied proxies (bed, etc.)
  for (let dx = 1; dx < wTiles; dx++) {
    if (!map.objects?.[y]?.[x + dx]) continue;
    map.objects[y][x + dx] = { id: "occupied", hp: 999, meta: { anchor: { x, y } } };
  }

  return true;
}

function placeInteriorObjectAt(x, y, objId, wTiles) {
  const map = state.interior;
  if (!map?.objects?.[y]) return;

  stampInteriorObject(map, x, y, objId, wTiles);

  // Don’t persist if this interior is meant to be hard-coded
  const idef = state.data?.interiors?.[state.interiorId];
...
let _largeFront = null;
  ...
  for (let y = camY; y < Math.min(h, camY + viewH); y++) {
    for (let x = camX; x < Math.min(w, camX + viewW); x++) {
     ...
     // Night: show objects/plant dirt if within player sight OR campfire light
...
if (o?.icon) {
  const isBuildStage = (obj.id === "build_site" || obj.id === "foundation" || obj.id === "framing" || obj.id === "house");

    if (isBuildStage) {
    const size = TILE_SIZE * BUILD_FOOTPRINT;
	const deferredWindowRays = [];
...
// --- Built interior walls: per-tile overlay (prevents clipping) ---
if (state.mode === "interior") {
  const map = state.interior;
  const walls = map?.walls;

  if (walls) {
    // Match your built-wall palette (same as drawBuiltInteriorWallsPass)
    const wallOuter = "#3e2a17";
    const wallInner = "#523621";
    const capWood = "rgba(180,120,60,0.60)";
    const capHi   = "rgba(255,255,255,0.18)";
    ...
    // IMPORTANT:
    // - Horizontal wall "belongs" to the TOP tile (x,y): walls.h[y][x]
    // - Vertical wall "belongs" to the LEFT tile (x,y): walls.v[y][x]
    // Drawing it HERE (after this tile's object) means:
    // - it can cover objects ON THIS TILE
    // - but objects on the tile below (drawn later) will be in front (no clipping)

       // Horizontal built wall at bottom edge of this tile
    if (walls.h && walls.h[y] && walls.h[y][x]) {
      // If player is on THIS tile, defer drawing until AFTER players so wall covers them
      if (playerTileSet.has(`${x},${y}`)) {
        deferredInteriorWallFront.push({ type: "h", sx, sy, band, inset, capH });
      } else {
        ...
        // Vertical built wall at right edge of this tile
    if (walls.v && walls.v[y] && walls.v[y][x]) {
      // If player is on THIS tile, defer drawing until AFTER players so wall covers them
      if (playerTileSet.has(`${x},${y}`)) {
        deferredInteriorWallFront.push({ type: "v", sx, sy, band, inset, capH });
      } else {
        const bx = sx + TILE_SIZE;
        const x0 = bx - band;
        drawRect(x0, sy, band, TILE_SIZE, wallOuter);
        drawRect(x0 + inset, sy + inset, band - inset * 2, TILE_SIZE - inset * 2, wallInner);
        drawRect(x0, sy, capH, TILE_SIZE, capWood);
        drawRect(x0, sy, 1, TILE_SIZE, capHi);
        drawRect(x0 + capH - 1, sy, 1, TILE_SIZE, capSh);
      }
    }
...
function _tileOfPlayer(pl) {
  // fx/fy are floats; use nearest tile
  return { x: Math.floor(pl.fx + 0.5), y: Math.floor(pl.fy + 0.5) };
}
...
if (state.mode === "overworld" && deferredLargeObjects.length) {
 ...
 // if (state.mode === "interior") drawBuiltInteriorWallsPass("back");
  ...
  if (hh) {
    ...
    // Deferred interior wall overlays AFTER players:
  // This makes the wall cover the player ONLY when they're on the wall's owning tile.
  if (state.mode === "interior" && deferredInteriorWallFront.length) {
    // Match palette
    const wallOuter = "#3e2a17";
    const wallInner = "#523621";
    const capWood = "rgba(180,120,60,0.60)";
    ...
    for (const w of deferredInteriorWallFront) {
      const band = w.band, inset = w.inset, capH = w.capH;

      if (w.type === "h") {
        const by = w.sy + TILE_SIZE;
        const y0 = by - band;
        drawRect(w.sx, y0, TILE_SIZE, band, wallOuter);
        drawRect(w.sx + inset, y0 + inset, TILE_SIZE - inset * 2, band - inset * 2, wallInner);
        drawRect(w.sx, y0, TILE_SIZE, capH, capWood);
        drawRect(w.sx, y0, TILE_SIZE, 1, capHi);
        drawRect(w.sx, y0 + capH - 1, TILE_SIZE, 1, capSh);
      } else {
        const bx = w.sx + TILE_SIZE;
        const x0 = bx - band;
        drawRect(x0, w.sy, band, TILE_SIZE, wallOuter);
        drawRect(x0 + inset, w.sy + inset, band - inset * 2, TILE_SIZE - inset * 2, wallInner);
        drawRect(x0, w.sy, capH, TILE_SIZE, capWood);
        drawRect(x0, w.sy, 1, TILE_SIZE, capHi);
        drawRect(x0 + capH - 1, w.sy, 1, TILE_SIZE, capSh);
      }
    }
  }
...
if (state.mode === "overworld" && _largeFront && _largeFront.length) {
  ...
  // if (state.mode === "interior") drawBuiltInteriorWallsPass("front");
}
